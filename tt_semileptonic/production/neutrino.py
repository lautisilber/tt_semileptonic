...
"""
Column producer for neutrino reconstruction, used by the ttbar chi2 reconstruction
(production/ttbar_reco.py). Ported from mtt/production/neutrino.py: reads the ``Lepton``
column directly instead of recomputing it, since ``production/lepton.py::lepton_producer``
already builds it during ``cf.SelectEvents`` and it is kept through ``cf.ReduceEvents``
(``Lepton.*`` in ``cfg.x.keep_columns``).
"""
