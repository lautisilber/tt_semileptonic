...
def _name_fn(categories: dict[str, od.Category]) -> str:
...
def _kwargs_fn(categories: dict[str, od.Category]) -> dict:
...
def add_categories_production(config: od.Config) -> None:
    ...
    create_category_combinations(config, category_groups, _name_fn, kwargs_fn=_kwargs_fn, parent_mode="all")
