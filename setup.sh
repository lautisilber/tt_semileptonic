...
setup_tt_semileptonic() {
    ...
    #   - "TT_SEMILEPTONIC_", for features provided by the analysis repository itself.
    # Check the setup.sh in columnflow for documentation of the "CF_" variables. The purpose of all
    # "TT_SEMILEPTONIC_" variables is documented below.
    #
    ...
    cf_export_bool TT_SEMILEPTONIC_SETUP
    if ${TT_SEMILEPTONIC_SETUP} && ! ${CF_ON_SLURM}; then
        >&2 echo "the tt_semileptonic analysis was already succesfully setup"
    ...
    #
    # global variables
    # (TT_SEMILEPTONIC = tt_semileptonic, CF = columnflow)
    #

    # start exporting variables
    export TT_SEMILEPTONIC_BASE="${this_dir}"
    export CF_BASE="${cf_base}"
    export CF_REPO_BASE="${TT_SEMILEPTONIC_BASE}"
    export CF_REPO_BASE_ALIAS="TT_SEMILEPTONIC_BASE"
    ...
    if ! ${CF_REMOTE_ENV}; then
        ...
        cf_setup_interactive "${CF_SETUP_NAME}" "${TT_SEMILEPTONIC_BASE}/.setups/${CF_SETUP_NAME}.sh" || return "$?"
    fi
    ...
    export PATH="${TT_SEMILEPTONIC_BASE}/bin:${PATH}"
    export PYTHONPATH="${TT_SEMILEPTONIC_BASE}:${TT_SEMILEPTONIC_BASE}/modules/cmsdb:${PYTHONPATH}"

    # initialze submodules
    if [ -e "${TT_SEMILEPTONIC_BASE}/.git" ]; then
        local m
        for m in $( ls -1q "${TT_SEMILEPTONIC_BASE}/modules" ); do
            cf_init_submodule "${TT_SEMILEPTONIC_BASE}" "modules/${m}"
        done
    fi
    ...
    export TT_SEMILEPTONIC_SETUP="true"
}
...
main() {
    ...
    if setup_tt_semileptonic "$@"; then
        cf_color green "tt_semileptonic analysis successfully setup"
        return "0"
    else
        local code="$?"
...
# entry point
if [ "${TT_SEMILEPTONIC_SKIP_SETUP}" != "true" ]; then
    main "$@"
fi
